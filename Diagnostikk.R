...
# Reprice at the optimum and show per-bucket residuals
best <- result$solution
pr   <- price_many_rB_turbo(options_df = options_df_calib[,c("type","K","T")],
                            S0=S0, r=r, q=q, alpha=best[1]-0.5, eta=best[3], rho=best[2],
                            xi0_curve=xi0_curve, N=NCal, M=MCal, antithetic=TRUE, seed=seed)
joined <- merge(transform(pr, type=tolower(type), K=K_round(K), T=T_round(T))[,c("type","K","T","price")],
                options_df_calib[,c("type","K","T","market_iv","maturity_bucket")],
                by=c("type","K","T"))
joined$iv_model <- mapply(function(typ,K,T,P) bs_implied_vol(P, S0,K,T,r,q,typ),
                          joined$type, joined$K, joined$T, joined$price)
joined$resid_iv <- joined$iv_model - joined$market_iv
cat(sprintf("IV RMSE: %.4f%%\n", 100*sqrt(mean(joined$resid_iv^2))))
print(joined %>% group_by(maturity_bucket) %>%
        summarise(n=dplyr::n(),
                  rmse_iv = 100*sqrt(mean(resid_iv^2)),
                  bias_iv = 100*mean(resid_iv),
                  .groups="drop"))
