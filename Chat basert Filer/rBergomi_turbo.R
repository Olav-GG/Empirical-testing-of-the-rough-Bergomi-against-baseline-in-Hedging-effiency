...
simulate_Y_paths_hybrid_k1_v3 <- function(
    M, N, T, H, b = 0.5, seed = NULL, antithetic = FALSE, ncores = 1
) {
...
# ---- Conditional Black???Scholes payoff pricing (helper) ----
.cond_bs_pathwise <- function(A, s2, Kd, type = c("call", "put"), tiny = 1e-14) {
  # Compute payoff for a path given conditional spot A and variance s2 (T=1 in BS world, r=q=0).
  type <- match.arg(type)
  s2  <- pmax(s2, 0)
  sd  <- sqrt(s2)
  z   <- log(pmax(A, tiny) / pmax(Kd, tiny))
  
  # d1, d2 in the Black???Scholes world (driftless since r=q=0)
  use_bs <- (sd > 0)
  d1 <- numeric(length(A)); d2 <- numeric(length(A))
  d1[use_bs] <- (z[use_bs] + 0.5 * s2[use_bs]) / sd[use_bs]
  d2[use_bs] <- d1[use_bs] - sd[use_bs]
  
  # Pathwise option payoff under conditional Black???Scholes
  out <- numeric(length(A))
  if (type == "call") {
    out[use_bs]  <- A[use_bs] * pnorm(d1[use_bs]) - Kd * pnorm(d2[use_bs])
    out[!use_bs] <- pmax(A[!use_bs] - Kd, 0)   # if zero variance, payoff = intrinsic
  } else {
