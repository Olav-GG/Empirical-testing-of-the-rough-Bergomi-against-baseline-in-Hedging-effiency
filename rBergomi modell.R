...
alpha <- 0
H   <- alpha + 0.5          # Hurst-parameter (rough hvis H < 0.5)
# Fremover-varianskurve skal v??re flat senere; vi parkerer niv??et:
xi0_level <- 0.235^2
eta <- 0
Rho <- 0
...
#batch_M  <- 20000
#n_batches <- ceiling(M_total / batch_M)

N=
M=10000
sim_hyb <- simulate_Y_paths_hybrid_k1(M = M, N = N, T = option$T, H = H,
...
#Prisings del:
rb_price_call_mc <- function(S0, K, r, q, rho, sim, v_mat, xi0_0, antithetic = FALSE) {
  t  <- sim$t; dt <- sim$dt
  N  <- length(t)
...
res <- rb_price_call_mc(S0 = S0, K = K, r = r, q = q,
                        ...
                        xi0_0 = xi0_level, antithetic = TRUE)
