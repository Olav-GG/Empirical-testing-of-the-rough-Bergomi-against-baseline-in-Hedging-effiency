...
nearest_idx <- function(x, target, used_pos = integer(0)) {
  ord <- order(abs(x - target))
  if (length(used_pos)) ord <- ord[!(ord %in% used_pos)]
  ord[1]
}
...
.build_G1_df <- function(df) {
  ...
  for (lab in names(target_Ts)) {
    ...
    if (!is.finite(Fsel)) Fsel <- S0 * exp((r - q) * Tsel)
    ...
    for (rat in ratio_map[[lab]]) {
      Ktgt <- rat * Fsel
      pos  <- nearest_idx(dfT$K[ok], Ktgt, used_pos = used_pos)
      idx  <- ok_pos[pos]
      used_pos <- c(used_pos, pos)
      ...
      rows[[length(rows)+1]] <- data.frame(
        type = typ, K = Km, T = Tm, ratio = rat,
        maturity_bucket = lab, fwd = Fsel, market_price = mkt_price,
